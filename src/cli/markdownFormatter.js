...
class MarkdownFormatter {
  ...
  format(result) {
    ...
    lines.push('');
    lines.push(
      `*Report generated by npm-security-score v${require('../../package.json').version}*`
    );
    lines.push('');
  ...
  _formatSummary(result) {
    const totalIssues = (result.ruleResults || []).filter(r => r.deduction > 0).length;
...
| **Rules Evaluated** | ${result.ruleResults?.length || 0} |`;
    ...
    ruleResults.forEach((rule, index) => {
      ...
      if (rule.details && rule.details.findings) {
        ...
        rule.details.findings.forEach(finding => {
          lines.push(
            `- **${finding.hook || 'Issue'}:** ${finding.description || 'Security concern detected'}`
          );
  ...
  _formatRecommendations(result) {
    ...
    if (recommendations.length === 0) {
      lines.push('✅ No specific recommendations. Package appears secure.');
      return lines.join('\n');
    }
